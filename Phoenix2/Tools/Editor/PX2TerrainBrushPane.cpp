...
TerrainBrushPanel::TerrainBrushPanel (wxWindow *parent)
{
	...
	mUseHelpGrid = (wxCheckBox*)FindWindow(XRCID("ShowHelpGrid"));
	assertion(mUseHelpGrid!=0, "Window must be find!");
...
{
	if (event.GetEventObject() == mUseHelpGrid)
	{
		PX2::Renderable *help = EditSystem::GetSingleton().GetTerrainEdit()
			->GetTerrainHelpGrid();

		if (mUseHelpGrid->IsChecked())
		{
			if (help)
				help->Culling = PX2::Movable::CULL_DYNAMIC;
		}
		else
		{
			if (help)
				help->Culling = PX2::Movable::CULL_ALWAYS;
		}
	}
}
//-----------------------------------------------------------------------------
