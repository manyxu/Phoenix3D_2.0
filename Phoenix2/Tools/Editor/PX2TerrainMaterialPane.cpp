...
void TerrainMaterialPanel::OnButton (wxCommandEvent& event)
{
	if (event.GetEventObject() == mTexToUseButton)
	{
		...
		if (EditSystem::GetSingleton().GetEditMode()
			== EditSystem::EM_TERRAIN_TEXTURE && tex)
		{
			EditSystem::GetSingleton().GetTerrainEdit()->GetTextureProcess()
				->SetSelectedLayerUsingTexture(tex);

			std::string strPath = tex->GetResourcePath();
			mTexToUse->Clear();
			mTexToUse->WriteText(strPath.c_str());
		}
		else
		{
			mTexToUse->Clear();
...
void TerrainMaterialPanel::RefleshCtrls ()
{
	...
	PX2::Event *ent = PX2::EditorEventSpace::CreateEvent(
		PX2::EditorEventSpace::SetTerrainTexturePreView);
	ent->SetData<PX2::Object*>(tex);
	PX2::EventWorld::GetSingleton().BroadcastingLocalEvent(ent);
	...
	if (selectedPage && terrain)
	{
		...
		EditSystem::GetSingleton().SetPreViewObject(tex2d);
	}
	else
	{
		mXIndex->Clear();
