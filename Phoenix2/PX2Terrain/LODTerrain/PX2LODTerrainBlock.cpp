											 ...
											 const Camera* camera)
{
	...
	// 整个地形坐标系下
	Box3f box;
	box.Center = page->LocalTransform.GetMatrix() * center;
	int i;
	for (i = 0; i < 3; i++)
	{
		HPoint axis;
	...
	// 用相机匹配相机截头体
	Frustum3f frustum;
	frustum.Origin = camera->GetPosition();
	frustum.DVector = camera->GetDVector();
	frustum.UVector = camera->GetUVector();
	frustum.RVector = camera->GetRVector();
