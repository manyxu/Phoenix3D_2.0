...
//----------------------------------------------------------------------------
void RawTerrainPage::SetTexture (int index, std::string filename)
{
	PX2::Texture2D *tex = DynamicCast<Texture2D>(PX2::ResourceManager
		::GetSingleton().BlockLoad(filename));
	assertion(tex!=0, "Load texture %s failed.", filename.c_str());

	SetTexture(index, tex);
}
//----------------------------------------------------------------------------
void RawTerrainPage::SetTexture0 (std::string filename)
{
	mTexture0 = DynamicCast<Texture2D>(PX2::ResourceManager::GetSingleton()
		.BlockLoad(filename));

	assertion(mTexture0!=0, "Load texture %s failed.", filename.c_str());
...
//----------------------------------------------------------------------------
void RawTerrainPage::SetTextureAlpha (std::string filename)
{
	mTextureAlpha = DynamicCast<Texture2D>(PX2::ResourceManager::GetSingleton()
		.BlockLoad(filename));

	assertion(mTextureAlpha!=0, "Load texture %s failed.", filename.c_str());

	if (mTextureAlpha)
	{
		mMaterialInstance->SetPixelTexture(0, 1, mTextureAlpha);
	}
}
//----------------------------------------------------------------------------
void RawTerrainPage::SetTexture1 (std::string filename)
{
	mTexture1 = DynamicCast<Texture2D>(PX2::ResourceManager::GetSingleton()
		.BlockLoad(filename));

	assertion(mTexture1!=0, "Load texture %s failed.", filename.c_str());
...
//----------------------------------------------------------------------------
void RawTerrainPage::SetTexture2 (std::string filename)
{
	mTexture2 = DynamicCast<Texture2D>(PX2::ResourceManager::GetSingleton()
		.BlockLoad(filename));

	assertion(mTexture2!=0, "Load texture %s failed.", filename.c_str());
...
//----------------------------------------------------------------------------
void RawTerrainPage::SetTexture3 (std::string filename)
{
	mTexture3 = DynamicCast<Texture2D>(PX2::ResourceManager::GetSingleton()
		.BlockLoad(filename));

	assertion(mTexture3!=0, "Load texture %s failed.", filename.c_str());
...
//----------------------------------------------------------------------------
void RawTerrainPage::SetTexture4 (std::string filename)
{
	mTexture4 = DynamicCast<Texture2D>(PX2::ResourceManager::GetSingleton()
		.BlockLoad(filename));

	assertion(mTexture4!=0, "Load texture %s failed.", filename.c_str());
...
//----------------------------------------------------------------------------
void RawTerrainPage::CreateInstancePerVertex (TerrainMaterial *material, 
											  Shine *shine)
{
	PX2::Light *light = 0;
...
//----------------------------------------------------------------------------
void RawTerrainPage::CreateInstancePerPixel (TerrainMaterial *material,
											 Shine *shine)
{
	PX2::Light *light = 0;
	for (int i=0; i<(int)mInfulencedLights.size(); i++)
	...
	mMaterialInstance->SetPixelConstant(0, "gShineEmissive",
		new0 ShineEmissiveConstant(shine));
	mMaterialInstance->SetPixelConstant(0, "gShineAmbient",
		new0 ShineAmbientConstant(shine));
	mMaterialInstance->SetPixelConstant(0, "gShineDiffuse",
		new0 ShineDiffuseConstant(shine));
