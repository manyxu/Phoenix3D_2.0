...
namespace PX2
{
	...
	*/
	class PX2_GRAPHICS_ENTRY Shader : public Object
	{
		PX2_DECLARE_RTTI;
		PX2_DECLARE_NAMES;
	...
	public:
		...
		// 着色器程序输入和输出变量类型
		enum PX2_GRAPHICS_ENTRY VariableType
		{
			VT_NONE,
		...
		// 输入输出变量的语义
		enum PX2_GRAPHICS_ENTRY VariableSemantic
		{
			VS_NONE,
			VS_POSITION,        // ATTR0
		...
		// 纹理采样器类型
		enum PX2_GRAPHICS_ENTRY SamplerType
		{
			ST_NONE,
			ST_1D,
			ST_2D,
		...
		// 纹理采样器纹理过滤模式
		enum PX2_GRAPHICS_ENTRY SamplerFilter
		{
			SF_NONE,
		...
		// 纹理寻址模式
		enum PX2_GRAPHICS_ENTRY SamplerCoordinate
		{
			SC_NONE,
			SC_CLAMP,
			SC_REPEAT,
