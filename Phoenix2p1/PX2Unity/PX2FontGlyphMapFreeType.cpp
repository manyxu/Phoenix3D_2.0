/*
*
* 文件名称	：	PX2FontGlyphMapFreeType.cpp
*
*/
...
{
	for(size_t i=0; i<(int)mFontFaceInfos.size(); i++)
	{
...
FT_Face FontGlyphMapFreeType::CreateFontFace(const char *fontName)
{
	...
	if (ResourceManager::GetSingleton().GetBuffer(info->Fontname,
		info->FaceBufferSize, info->FaceBuffer))
	{
		int error = FT_New_Memory_Face(msFTLibrary, 
			(const unsigned char*)info->FaceBuffer, 
			(FT_Long)info->FaceBufferSize, 0, &(info->Face));
