...
mSelectMode(SM_MAX_MODE)
{
	...
	VertexFormat *vFormat = EditSystem::GetSingleton().GetCurveEdit()
		->GetVertexFormat();
...
bool CurveCtrlFloat::SetInVal (float inVal)
{
	...
	if (newIndex != oldIndex)
	{
		...
		EditSystem::GetSingleton().GetCurveEdit()->SetSelectedCurveCtrl(ctrl);
	}
...
bool CurveCtrlFloat3Axis::SetArriveTangent (PX2::AVector at)
{
	...
	int axis = mCurveFloat3Axis->GetAxis();
	mCurveFloat3Axis->mCurve->Points[mIndex].ArriveTangent[axis] 
		= mArriveTangent.Z()/mArriveTangent.X();
	mCurveFloat3Axis->mCurve->Points[mIndex].LeaveTangent[axis]
		= mLeaveTangent.Z()/mLeaveTangent.X();
...
void CurveCtrlFloat3Axis::OnCtrlChanged (bool updateCurve)
{
	...
	AVector lt (1.0f, 0.0f, arriveTangent[axis]);
	lt.Normalize();
	mLeaveTangent = lt;
