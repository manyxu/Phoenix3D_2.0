...
void Curve::SetVisible (bool show)
{
	...
	mNode->Culling = show ? Movable::CULL_DYNAMIC : Movable::CULL_ALWAYS;
}
//----------------------------------------------------------------------------
...
mCurve(curve)
{
	...
	mPoly = EditSystem::GetSingleton().GetCurveEdit()->CreatePoly(curve, 
		mCurveColor);
...
{
	EditSystem::GetSingleton().GetCurveEdit()->UpdatePoly(mCurve, mPoly, 
		mCurveColor);
}
//----------------------------------------------------------------------------
...
void CurveFloat::ReCreateCtrls ()
{
	...
	float xScale = EditSystem::GetSingleton().GetCurveEdit()->GetCtrlsScaleX();
	float zScale = EditSystem::GetSingleton().GetCurveEdit()->GetCtrlsScaleZ();
...
// CurveFloat3
//----------------------------------------------------------------------------
CurveFloat3::CurveFloat3 (PX2::InterpCurveFloat3 *curve, Float3 color,
	bool isForInit)
	:
Curve(isForInit),
mCurve(curve)
...
mAxis(axis)
{
	...
	mPoly = EditSystem::GetSingleton().GetCurveEdit()->CreatePoly(curve, axis, 
		mCurveColor);
	mNode->AttachChild(mPoly);
...
{
	EditSystem::GetSingleton().GetCurveEdit()->UpdatePoly(mCurve, mPoly, mAxis,
		mCurveColor);
}
//----------------------------------------------------------------------------
