/*
*
* 文件名称	：	PX2SceneBuilderAnimation.cpp
*
*/
...
void SceneBuilder::BuildKeyFrameController(INode *node, PX2::Movable *movable)
{
	...
	for (int i=0; i<(int)keyInfos.size(); i++)
	{
		...
		case KT_SCALE:
			...
			{
				scaleMax = trans.GetUniformScale();
			}
			else
			{
				scaleMax = PX2::Mathf::FAbs(trans.GetScale().X());
				float scaleAbs = PX2::Mathf::FAbs(trans.GetScale().Y());
				if (scaleAbs > scaleMax)
					scaleMax = scaleAbs;
				scaleAbs = PX2::Mathf::FAbs(trans.GetScale().Z());
				if (scaleAbs > scaleMax)
					scaleMax = scaleAbs;
			}
